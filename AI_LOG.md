...
## 2026-05-26
- **Cambios realizados:** Se refinó y expandió el soporte de servicios en `config/knowledgeBase.js`, actualizando las respuestas para el correo institucional (aka.ms/sspr), LMS Canvas, WiFi SoyUdeMedellín y red Porterías, login de SIGAA con cédula y fecha de nacimiento por defecto, carné digital mediante la App UdeMedellín, servicios de Bienestar Universitario en el Bloque 2 (citas de salud gratuitas), Biblioteca Eduardo Fernández Botero (horario 6 a.m. a 6 p.m. y ext. 11416), Centro de Desarrollo Empresarial (Bloque 15-103) y métodos de pago de Tesorería (PSE y convenios Bancolombia/Caja Social).
...
- **Validacion realizada:** `npm.cmd run build` y `npm.cmd run lint` ejecutados con resultado correcto tras expandir/refinar la base de conocimiento y eliminar el panel de configuracion.
- **Decisiones tecnicas:**
